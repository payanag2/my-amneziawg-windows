...
:build
	...
	echo [+] Signing
	signtool sign %SigningProvider% /fd sha256 /tr "%TimestampServer%" /td sha256 /d "MyAmneziaWG Setup" "dist\MyAmneziaWG-*-Windows-%WIREGUARD_VERSION%.msi" || goto :error
...
:msi
	...
	echo [+] Signing %1
	signtool sign %SigningProvider% /fd sha256 /tr "%TimestampServer%" /td sha256 /d "MyAmneziaWG Setup Custom Actions" "%~1\customactions.dll" || exit /b 1
:skipsign
	"%WIX%bin\candle" %WIX_CANDLE_FLAGS% -dWIREGUARD_PLATFORM="%~1" -out "%~1\wireguard.wixobj" -arch %3 wireguard.wxs || exit /b %errorlevel%
	echo [+] Linking %1
	"%WIX%bin\light" %WIX_LIGHT_FLAGS% -out "dist\MyAmneziaWG-%~1-Windows-%WIREGUARD_VERSION%.msi" "%~1\wireguard.wixobj" || exit /b %errorlevel%
	goto :eof
